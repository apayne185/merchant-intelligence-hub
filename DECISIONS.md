...
> **Nota de numeración**: Esta entrada se escribió en paralelo a otra rama
> (`feature/otel-tracing`, PR pendiente) que ya reclamó D37 para su propio
> trabajo. Numerado D38 asumiendo que esa PR se mergea primero — si esta
> PR (ingestion) se mergea primero en cambio, quien mergee la otra rama
> después necesita renumerar esa entrada (o esta) para evitar un D-número
> duplicado en `main`. Un recordatorio de una limitación real de trabajar
> en ramas paralelas con un log de decisiones secuencial.
> *Numbering note: This entry was written in parallel with another branch
> (`feature/otel-tracing`, PR pending) that already claimed D37 for its
> own work. Numbered D38 assuming that PR merges first — if this PR
> (ingestion) merges first instead, whoever merges the other branch
> afterward needs to renumber that entry (or this one) to avoid a
> duplicate D-number landing on `main`. A reminder of a real limitation of
> working on parallel branches with a sequential decision log.*
