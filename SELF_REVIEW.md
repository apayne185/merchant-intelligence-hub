...
- **Actualización**: se encontró y corrigió un bug concreto derivado de este mismo problema — el bloque T5 de `quality_report` re-parseaba `transaction_date` en una sola pasada (sin el segundo intento `dayfirst=True` que sí hace `load_clean`), y no imputaba `amount` nulo antes de deduplicar. El conteo de duplicados reportado (4351) no coincidía con lo que `load_clean` realmente elimina (4182). Se corrigió replicando exactamente la misma lógica de dos pasadas + imputación, y se agregó un test (`test_quality_report_dup_count_matches_load_clean`) que falla si vuelven a desincronizarse. La causa raíz (dos implementaciones independientes del mismo parseo) sigue sin resolverse — el `transforms_log` propuesto arriba sigue siendo la solución correcta a largo plazo.
- *Update: found and fixed a concrete bug stemming from this same issue — quality_report's T5 block re-parsed `transaction_date` in a single pass (missing the second `dayfirst=True` retry that `load_clean` does), and didn't impute null `amount` before deduplicating. The reported duplicate count (4351) didn't match what `load_clean` actually removes (4182). Fixed by replicating the exact same two-pass-parsing + imputation logic, and added a test (`test_quality_report_dup_count_matches_load_clean`) that fails if they desync again. The root cause (two independent implementations of the same parsing) is still unresolved — the `transforms_log` proposed above remains the correct long-term fix.*
